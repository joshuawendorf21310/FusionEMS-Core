...
def upgrade() -> None:
    ...
    with op.batch_alter_table("tenants") as batch:
        batch.add_column(
            sa.Column(
                "billing_tier", sa.String(length=64), server_default="starter", nullable=False
            )
        )
        batch.add_column(
            sa.Column(
        ...
        batch.add_column(
            sa.Column(
                "billing_status", sa.String(length=32), server_default="inactive", nullable=False
            )
        )
        batch.add_column(
            sa.Column(
    ...
    op.create_index(
        "ix_accreditation_items_tenant", "accreditation_items", ["tenant_id", "category"]
    )
    ...
    op.create_index(
        "ix_accreditation_evidence_item", "accreditation_evidence", ["tenant_id", "item_id"]
    )
