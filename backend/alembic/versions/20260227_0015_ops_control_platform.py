...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_cases_transport_mode ON cases USING gin((data->'transport_mode'));")
    op.execute("CREATE INDEX IF NOT EXISTS ix_readiness_unit_id ON readiness_scores USING gin((data->'unit_id'));")
    op.execute("CREATE INDEX IF NOT EXISTS ix_maintenance_unit_id ON maintenance_work_orders USING gin((data->'unit_id'));")
    op.execute("CREATE INDEX IF NOT EXISTS ix_hems_acceptance_mission ON hems_acceptance_records USING gin((data->'mission_id'));")
