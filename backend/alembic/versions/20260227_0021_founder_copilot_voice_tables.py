...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("session_id", UUID(as_uuid=True), sa.ForeignKey("founder_chat_sessions.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("run_id", UUID(as_uuid=True), sa.ForeignKey("founder_chat_runs.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("popped_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
    ...
    op.create_index("ix_voice_compliance_guard_call", "voice_compliance_guard_events", ["call_control_id"])
    ...
    op.create_index("ix_voice_callback_slots_status", "voice_callback_slots", ["status", "scheduled_at"])
    ...
    op.create_index("ix_voice_preferences_tenant_phone", "voice_preferences", ["tenant_id", "caller_phone"])
    ...
    op.create_index("ix_voice_recording_governance_tenant", "voice_recording_governance", ["tenant_id"])
    ...
    op.create_index("ix_voice_recording_access_log_tenant", "voice_recording_access_log", ["tenant_id", "accessed_at"])
    ...
    op.create_index("ix_voice_human_review_status", "voice_human_review_queue", ["status", "created_at"])
...
def downgrade() -> None:
    for table in [
        "invoices", "support_tickets", "tenant_users",
        "voice_improvement_tickets", "voice_human_review_queue",
        "voice_war_room_incidents", "voice_recording_access_log",
        "voice_recording_governance", "voice_preferences",
        "voice_cost_caps", "voice_ab_tests", "voice_callback_slots",
        "voice_founder_busy_states", "voice_onboarding_sessions",
        "voice_compliance_guard_events", "voice_script_packs",
        "voice_alert_policies", "voice_screen_pops",
