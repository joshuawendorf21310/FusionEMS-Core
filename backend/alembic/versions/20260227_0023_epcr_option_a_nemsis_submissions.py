...
def upgrade() -> None:
    ...
    for col_name, col_def in [
        ...
        ("schema_version",    sa.Column("schema_version", sa.String(16), nullable=False, server_default=sa.text("'3.5'"))),
    ...
    conn.execute(sa.text(
        "UPDATE epcr_charts SET status = data->>'chart_status' WHERE status IS NULL"
    ))
    ...
    if not _col_exists(conn, "epcr_event_log", "deleted_at"):
        op.add_column("epcr_event_log", sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True))
    conn.execute(sa.text(
    ...
    if not _table_exists(conn, "nemsis_submission_status_history"):
        ...
        op.create_index("ix_nssh_submission_id", "nemsis_submission_status_history", ["submission_id"])
        conn.execute(sa.text(
...
def downgrade() -> None:
    ...
    for t in ["nemsis_submission_status_history", "nemsis_submission_results"]:
        if t in sa.inspect(conn).get_table_names():
            op.drop_table(t)
    for col in ["status", "submitted_at", "deleted_at", "legal_hold",
