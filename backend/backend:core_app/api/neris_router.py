...
async def validate(payload: dict[str, Any], request: Request, current: CurrentUser = Depends(get_current_user), db: Session = Depends(db_session_dependency)):
    ...
    return await svc.create(table="neris_validation_results", tenant_id=current.tenant_id, actor_user_id=current.user_id, data=result, correlation_id=getattr(request.state,"correlation_id",None))
...
async def create_export(payload: dict[str, Any], request: Request, current: CurrentUser = Depends(get_current_user), db: Session = Depends(db_session_dependency)):
    svc = DominationService(db, get_event_publisher())
    job = {"status":"queued","range":payload.get("range")}
    return await svc.create(table="neris_export_jobs", tenant_id=current.tenant_id, actor_user_id=current.user_id, data=job, correlation_id=getattr(request.state,"correlation_id",None))
