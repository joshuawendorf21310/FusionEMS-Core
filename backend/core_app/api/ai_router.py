...
@router.post("/chat")
def chat(payload: dict, db: Session = Depends(db_session_dependency), user: CurrentUser = Depends(get_current_user)):
    ...
    try:
        db.execute(
            ...
            {"tid": str(user.tenant_id), "model": meta.get("model",""), "pv":"v1", "ih": hash_input(prompt), "tc": json.dumps({"usage": meta.get("usage",{})})},
        )
        db.commit()
    except Exception:
        db.rollback()
