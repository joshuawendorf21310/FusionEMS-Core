...
def export_fhir(
    ...
    resource_type, resource = map_to_fhir(
        db, user.tenant_id, payload.entity_type, payload.entity_id
    )
    row = (
        db.execute(
            text("""
    ...
    return FhirExportResponse(
        artifact_id=str(row["id"]), resource_type=resource_type, resource=resource
    )
