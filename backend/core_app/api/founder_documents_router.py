    ...
    for row in rows:
        ...
        if s3_key and settings.s3_bucket_docs:
            try:
                ...
                presigned_url = presign_get(bucket=settings.s3_bucket_docs, key=s3_key, expires_seconds=300)
...
async def resend_legal(
    ...
    db.execute(
        text("UPDATE onboarding_applications SET status = 'legal_pending', legal_status = 'pending' WHERE id = :app_id"),
        {"app_id": application_id},
    )
    db.commit()
...
async def resend_checkout(
    ...
    if app_row["legal_status"] != "signed":
        raise HTTPException(status_code=422, detail="Legal documents must be signed before checkout")
...
async def manual_provision(
    ...
    if not payload.get("confirm"):
        raise HTTPException(status_code=422, detail="confirm: true is required to manually trigger provisioning")
    ...
    if app_row is None:
        raise HTTPException(status_code=404, detail="Application not found")
    if app_row["legal_status"] != "signed":
        raise HTTPException(status_code=422, detail="Legal documents must be signed before provisioning")
    ...
    try:
        result = await provision_tenant_from_application(
            db, application_id, dict(app_row), {"type": "manual_provision", "source": "founder_override"}
        )
    except Exception as exc:
