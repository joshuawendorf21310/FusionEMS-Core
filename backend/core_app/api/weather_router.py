...
async def layers(
    ...
    return {
        "radar_tiles": {"template": "https://example.com/tiles/{z}/{x}/{y}.png"},
        "alerts": True,
    }
...
async def aviation(
    ...
    svc = DominationService(db, get_event_publisher())
    return svc.repo("aviation_weather_reports").list(
...
async def refresh(
    ...
    return await svc.create(
        table="weather_tiles_cache",
        tenant_id=current.tenant_id,
        actor_user_id=current.user_id,
        data={"action": "refresh", "payload": payload},
