...
def _header_plate(c: Any, agency_name: str) -> None:
    ...
    c.drawRightString(
        PAGE_W - 0.65 * inch, plate_y + plate_h * 0.62, "MEDICAL TRANSPORT BILLING STATEMENT"
    )
    c.setFillColor(TICK_GREY)
    c.setFont("Courier", 6)
    c.drawRightString(
        PAGE_W - 0.65 * inch, plate_y + plate_h * 0.30, "CONFIDENTIAL — HIPAA PROTECTED"
    )
...
def _build_page1(c: Any, ctx: StatementContext, pdf_hash: str) -> None:
    ...
    labels = [
        (
            "Statement ID",
            ctx.statement_id[:18] + "…" if len(ctx.statement_id) > 18 else ctx.statement_id,
        ),
        ("Incident Date", ctx.incident_date),
        ("Transport Date", ctx.transport_date),
        (
            "Due Date",
            (ctx.generated_at + __import__("datetime").timedelta(days=30)).strftime("%m/%d/%Y"),
        ),
    ]
    ...
    # Table header row
    _chamfer_rect(
        c, tbl_x, cur_y - row_h, tbl_w, row_h, cut=3.0, fill_color=CHARCOAL_MID, stroke_color=None
    )
    rx = tbl_x + 0.05 * inch
    c.setFillColor(OFF_WHITE)
    ...
    _chamfer_rect(
        c, bal_x, bal_y, bal_w, bal_h, cut=7.0, fill_color=plate_color, stroke_color=ORANGE
    )
    c.setFillColor(OFF_WHITE)
...
def _build_page2(c: Any, ctx: StatementContext, pdf_hash: str) -> None:
    ...
    instructions = [
        ("Online (fastest):", f"Visit {ctx.pay_url}  —  Secure card payment via Stripe Checkout."),
        (
            "By Text/SMS:",
    ...
    _chamfer_rect(
        ...
        1.60 * inch,
        cut=5.0,
        fill_color=CHARCOAL_LITE,
        stroke_color=TICK_GREY,
    )
