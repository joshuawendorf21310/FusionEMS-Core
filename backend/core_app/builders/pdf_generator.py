...
def generate_roi_proposal_pdf(roi_data: dict[str, Any], agency_info: dict[str, Any]) -> bytes:
    ...
    title_style = ParagraphStyle("title", parent=styles["Heading1"], fontSize=24, textColor=BRAND_DARK,
                                  spaceAfter=6)
    subtitle_style = ParagraphStyle("subtitle", parent=styles["Normal"], fontSize=12, textColor=BRAND_BLUE,
                                     spaceAfter=20)
    section_style = ParagraphStyle("section", parent=styles["Heading2"], fontSize=14, textColor=BRAND_DARK,
    ...
    roi_table_data = [
        ...
        ["Gross Revenue", f"${gross/100:,.0f}", f"${(gross+uplift)/100:,.0f}", f"+${uplift/100:,.0f}"],
        ["Denial Rate", f"{roi_data.get('current_denial_rate', 12):.0f}%",
         f"{roi_data.get('projected_denial_rate', 6):.0f}%",
         f"-{roi_data.get('current_denial_rate', 12) - roi_data.get('projected_denial_rate', 6):.0f}%"],
        ["Recovery from Denials", "—", f"${denial_reduction/100:,.0f}", f"+${denial_reduction/100:,.0f}"],
    ]
    ...
    features = [
        ("Overlay Mode", "Keep your existing ePCR. Plug in billing intelligence without rip-and-replace."),
        ("Transparent Pricing", "Base fee + per-transport. No percentage cuts. See exactly what you pay."),
        ...
        ("Self-Service Onboarding", "ROI → Proposal → Sign → Pay → Deploy. No sales calls required."),
    ]
...
def generate_contract_pdf(
    ...
    story.append(Paragraph("FusionEMS Quantum", ParagraphStyle("h1", parent=styles["Heading1"], fontSize=20, textColor=BRAND_DARK)))
    story.append(Paragraph("Master Subscription Agreement", ParagraphStyle("h2", parent=styles["Heading2"], fontSize=14)))
    ...
    story.append(Paragraph("Subscription Terms", ParagraphStyle("h3", parent=styles["Heading3"], fontSize=12, textColor=BRAND_DARK)))
    ...
    story.append(Paragraph("Terms", ParagraphStyle("h3", parent=styles["Heading3"], fontSize=12, textColor=BRAND_DARK)))
    terms_text = [
...
def generate_baa_pdf(agency_info: dict[str, Any]) -> bytes:
    ...
    story.append(Paragraph("BUSINESS ASSOCIATE AGREEMENT", ParagraphStyle("h1", parent=styles["Heading1"], fontSize=18, textColor=BRAND_DARK, alignment=1)))
    story.append(HRFlowable(width="100%", thickness=1, color=BRAND_BLUE))
    story.append(Spacer(1, 0.2*inch))

    story.append(Paragraph(f"This Business Associate Agreement ('BAA') is entered into between {agency_info.get('name', 'Covered Entity')} ('Covered Entity') and FusionEMS Quantum ('Business Associate').", body))
    story.append(Spacer(1, 0.1*inch))

    baa_sections = [
        ("1. Definitions", "Terms used herein shall have the meanings set forth in 45 CFR §160.103 and §164.304."),
        ("2. Permitted Uses", "Business Associate may use and disclose PHI only as necessary to provide the services described in the underlying agreement."),
        ("3. Safeguards", "Business Associate shall implement appropriate administrative, physical, and technical safeguards to protect the confidentiality, integrity, and availability of PHI."),
        ("4. Breach Notification", "Business Associate shall notify Covered Entity without unreasonable delay, and no later than 60 days, following discovery of a breach of unsecured PHI."),
        ("5. Term and Termination", "This BAA is effective upon signing and remains in effect until the underlying service agreement is terminated."),
        ("6. Return/Destruction of PHI", "Upon termination, Business Associate shall return or destroy all PHI, if feasible."),
    ]
    for title, text in baa_sections:
        story.append(Paragraph(title, ParagraphStyle("h3", parent=styles["Heading3"], fontSize=11, textColor=BRAND_DARK)))
        story.append(Paragraph(text, body))

    story.append(Spacer(1, 0.5*inch))
    story.append(Paragraph("IN WITNESS WHEREOF, the parties have executed this BAA as of the date signed below.", body))
    story.append(Spacer(1, 0.3*inch))
