...
class NEMSISExporter:
    def _sub(
        self, parent: Element, tag: str, text: str | None = None, attrib: dict | None = None
    ) -> Element:
    ...
    def export_chart(self, chart_dict: dict[str, Any], agency_info: dict[str, Any]) -> bytes:
        ...
        primary = next(
            (a for a in assessments if a.get("assessment_type") == "primary"),
        ...
        self._sub(
            eresponse,
            self._ns("eResponse.23"),
            TRANSPORT_MODE_MAP.get(disposition.get("transport_mode", ""), NV_NOT_RECORDED),
        )
        self._sub(
            eresponse,
            self._ns("eResponse.28"),
            LEVEL_OF_CARE_MAP.get(disposition.get("level_of_care", ""), NV_NOT_RECORDED),
        )
        ...
        self._sub(
            etimes, self._ns("eTimes.07"), self._fmt_time(dispatch.get("patient_contact_time"))
        )
        self._sub(
            etimes, self._ns("eTimes.09"), self._fmt_time(dispatch.get("departed_scene_time"))
        )
        self._sub(
            etimes, self._ns("eTimes.11"), self._fmt_time(dispatch.get("arrived_destination_time"))
        )
        self._sub(
            etimes, self._ns("eTimes.12"), self._fmt_time(dispatch.get("transfer_of_care_time"))
        )
        ...
        self._sub(
            epatient,
            self._ns("ePatient.13"),
            GENDER_MAP.get((patient.get("gender") or "").lower(), NV_NOT_RECORDED),
        )
        self._sub(
            epatient,
            self._ns("ePatient.14"),
            RACE_MAP.get((patient.get("race") or "").lower(), NV_NOT_RECORDED),
        )
        ...
        self._sub(
            esituation,
            self._ns("eSituation.13"),
            self._nv(medical_findings.get("acuity"))
            if isinstance(medical_findings, dict)
        ...
        self._sub(
            ehistory,
            self._ns("eHistory.01"),
            "; ".join(allergies) if allergies else NV_NOT_RECORDED,
        )
        meds_home = primary.get("medications_home", [])
        self._sub(
            ehistory,
            self._ns("eHistory.08"),
            "; ".join(meds_home) if meds_home else NV_NOT_RECORDED,
        )
        ...
        for med in chart_dict.get("medications", []):
            ...
            self._sub(
                mg,
                self._ns("eMedications.10"),
                "9909003" if med.get("prior_to_our_care") else "9909001",
            )
        ...
        for proc in chart_dict.get("procedures", []):
            ...
            self._sub(
                pg, self._ns("eProcedures.07"), "9923001" if proc.get("successful") else "9923003"
            )
            self._sub(pg, self._ns("eProcedures.08"), self._nv(proc.get("complications")))
            self._sub(
                pg,
                self._ns("eProcedures.10"),
                "9909003" if proc.get("prior_to_our_care") else "9909001",
            )
        ...
        self._sub(
            enarrative,
            self._ns("eNarrative.01"),
            narrative if narrative and narrative.strip() else NV_NOT_RECORDED,
        )

        edisposition = self._sub(record, self._ns("eDisposition"))
        self._sub(
            edisposition,
            self._ns("eDisposition.12"),
            self._nv(disposition.get("patient_disposition_code")),
        )
        self._sub(
            edisposition, self._ns("eDisposition.16"), self._nv(disposition.get("destination_name"))
        )
        self._sub(
            edisposition,
            self._ns("eDisposition.27"),
            self._nv(disposition.get("patient_disposition_code")),
        )
        self._sub(
            edisposition,
            self._ns("eDisposition.28"),
            self._nv(disposition.get("transport_disposition")),
        )
        ...
        self._sub(
            eincident,
            self._ns("eIncident.01"),
            incident_number if incident_number and incident_number.strip() else NV_NOT_RECORDED,
        )
