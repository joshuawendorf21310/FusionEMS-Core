...
def map_patient(p: Patient) -> dict:
    ...
    return {
        ...
        "id": _fhir_id(p.id),
        "identifier": [{"system": "urn:fusionems:patient", "value": p.external_identifier}]
        if p.external_identifier
        else [],
        "name": [name],
...
def map_incident_to_encounter(i: Incident) -> dict:
    ...
    return {
        ...
        "status": "in-progress"
        if i.status.value in ("draft", "in_progress", "ready_for_review")
        else "finished",
        "class": {
