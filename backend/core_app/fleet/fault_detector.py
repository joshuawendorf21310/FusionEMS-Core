...
OBD_FAULT_THRESHOLDS = {
    "coolant_temp_c": {
        "max": 110,
        "severity": "critical",
        "message": "Engine coolant temperature critical ({value}°C). Stop vehicle immediately.",
    },
    "engine_rpm": {"max": 6000, "severity": "warning", "message": "Engine RPM high ({value} RPM)."},
    "battery_voltage": {
        "min": 11.5,
        "severity": "warning",
        "message": "Battery voltage low ({value}V). Check charging system.",
    },
    "fuel_level_pct": {
        "min": 10,
        "severity": "warning",
        "message": "Fuel level critically low ({value}%).",
    },
    "oil_pressure_kpa": {
        "min": 100,
        "severity": "critical",
        "message": "Oil pressure critically low ({value} kPa). Stop vehicle immediately.",
    },
    "throttle_position_pct": {
        "max": 99,
        "severity": "info",
        "message": "Throttle at maximum ({value}%).",
    },
    "speed_kmh": {
        "max": 160,
        "severity": "warning",
        "message": "Vehicle speed excessive ({value} km/h).",
    },
}
...
class FaultDetector:
    ...
    def analyze_obd_reading(
        self, unit_id: uuid.UUID, obd_payload: dict[str, Any]
    ) -> list[dict[str, Any]]:
        alerts = []
        ...
        if rpm is not None and speed is not None:
            try:
                if (
                    IDLE_RPM_MIN <= float(rpm) <= IDLE_RPM_MAX
                    and float(speed) <= IDLE_SPEED_MAX_KMH
                ):
                    alerts.append(
                        {
                            "unit_id": str(unit_id),
