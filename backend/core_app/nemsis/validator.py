...
class NEMSISValidator:
    ...
    def _stage_national_schematron(self, root: ET.Element) -> list[ValidationIssue]:
        ...
        checks = [
            ("eRecord.01", "NEMSIS-001", "PCR Report Number (eRecord.01) is missing or empty.",
             "eRecord.01 element not found or has no text content.",
             "National NEMSIS", "Add a unique PCR report number to eRecord.01."),
            ("eIncident.01", "NEMSIS-002", "Incident Number (eIncident.01) is missing or empty.",
             "eIncident.01 element not found.",
             "National NEMSIS", "Populate eIncident.01 with the incident number."),
            ("eTimes.01", "NEMSIS-003", "PSAP Call Time (eTimes.01) is required but missing.",
             "eTimes.01 element not found.",
             "National NEMSIS", "Set eTimes.01 to the PSAP call date/time in ISO 8601 format."),
            ("eTimes.03", "NEMSIS-004", "Unit Notified Time (eTimes.03) is required but missing.",
             "eTimes.03 element not found.",
             "National NEMSIS", "Set eTimes.03 to the unit notified date/time."),
            ("eNarrative.01", "NEMSIS-008", "Narrative (eNarrative.01) is required but missing.",
             "eNarrative.01 element not found.",
             "National NEMSIS", "Add a patient care narrative to eNarrative.01."),
            ("eDisposition.27", "NEMSIS-009", "Final Patient Acuity (eDisposition.27) is required but missing.",
             "eDisposition.27 element not found.",
             "National NEMSIS", "Set eDisposition.27 to the final patient acuity code."),
        ]
        ...
        for element_id, rule_id, plain_msg, tech_msg, source, hint in checks:
            ...
            if val is None:
                issues.append(_make_issue("error", "national_schematron", rule_id, element_id,
                                          plain_msg, tech_msg, source, hint))
    ...
    def _stage_wi_schematron(self, root: ET.Element) -> list[ValidationIssue]:
        ...
        wi_checks = [
            ("eTimes.06", "WI-001", "Wisconsin requires Arrived Scene Time (eTimes.06).",
             "eTimes.06 not found.", "Set eTimes.06 to the unit arrived on scene date/time."),
            ("eTimes.07", "WI-002", "Wisconsin requires Patient Contact Time (eTimes.07).",
             "eTimes.07 not found.", "Set eTimes.07 to the patient contact date/time."),
            ("eResponse.13", "WI-003", "Wisconsin requires Unit Call Sign (eResponse.13).",
             "eResponse.13 not found.", "Populate eResponse.13 with the unit's call sign."),
            ("eSituation.11", "WI-004", "Wisconsin requires Primary Impression (eSituation.11).",
             "eSituation.11 not found.", "Select a provider primary impression code for eSituation.11."),
            ("eResponse.23", "WI-005", "Wisconsin requires Transport Mode (eResponse.23).",
             "eResponse.23 not found.", "Set eResponse.23 to the appropriate transport mode code."),
        ]

        for element_id, rule_id, plain_msg, tech_msg, hint in wi_checks:
            val = _find_text(root, element_id)
            if val is None:
                issues.append(_make_issue("error", "wi_schematron", rule_id, element_id,
                                          plain_msg, tech_msg, "Wisconsin", hint))
