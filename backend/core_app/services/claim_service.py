...
SENSITIVE_AUDIT_FIELDS = {"name", "dob", "narrative", "note", "phone", "signature", "transcript", "address"}
...
class ClaimService:
    ...
    async def create_claim(
        ...
        *,
        ...
        claim = Claim(tenant_id=tenant_id, status=ClaimStatus.DRAFT, version=1, idempotency_key=idempotency_key, **payload.model_dump())
        ...
        await self._write_audit_log(
            ...
            created.id,
            "claim.created",
            list(payload.model_dump().keys()) + ["status", "idempotency_key"],
            correlation_id,
        )
        await self.db.commit()
        await self.publisher.publish("claim.created", tenant_id, created.id, {"status": created.status.value})
    ...
    async def update_claim(
        ...
        *,
        ...
        await self.db.commit()
        await self.publisher.publish("claim.updated", tenant_id, updated.id, {"changed_fields": self._sanitize_field_names(changed)})
    ...
    async def transition_claim(
        ...
        *,
        ...
        if payload.target_status not in allowed:
            raise AppError(
                ...
                details={
                    ...
                    "allowed_targets": [s.value for s in sorted(allowed, key=lambda s: s.value)],
                },
            )
        ...
        await self.publisher.publish(
            ...
            tenant_id,
            updated.id,
            {"from_status": from_status.value, "to_status": updated.status.value},
        )
        return ClaimResponse.model_validate(updated)
    ...
    @staticmethod
    def _sanitize_field_names(changed_fields: list[str]) -> list[str]:
        sanitized: list[str] = []
    ...
    async def _write_audit_log(
        ...
        self.db.add(
            AuditLog(
                ...
                entity_name="claim",
                entity_id=entity_id,
                field_changes={"changed_fields": self._sanitize_field_names(changed_fields), "metadata": {}},
