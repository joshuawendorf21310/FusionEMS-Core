...
class ClaimService:
    ...
    async def create_claim(
        ...
        *,
        ...
        await self._write_audit_log(
            ...
            changed_fields=list(payload.model_dump().keys()) + ["status"],
        ...
        if payload.target_status not in allowed:
            raise AppError(
                ...
                details={
                    ...
                    "allowed_targets": [
                        status.value for status in sorted(allowed, key=lambda item: item.value)
                    ],
                },
            )
    ...
    async def _require_claim(self, *, tenant_id: uuid.UUID, claim_id: uuid.UUID) -> Claim:
        ...
        if claim is None:
            raise AppError(
                code=ErrorCodes.CLAIM_NOT_FOUND,
