...
class TestLobSignatureVerification:
    ...
    def test_valid_signature_passes(self):
        ...
        assert verify_lob_webhook_signature(
            raw_body=body,
...
class TestStripeSignatureVerification:
    ...
    def _make_event(self, event_type: str = "payment_intent.succeeded") -> bytes:
        return json.dumps({
            ...
            "type": event_type,
            "data": {"object": {"id": f"pi_{uuid.uuid4().hex[:24]}", "metadata": {"statement_id": str(uuid.uuid4())}}},
        }).encode()
...
class TestPaymentSuccessFlow:
    def _run_stripe_event(self, message: dict) -> tuple[MagicMock, MagicMock]:
        ...
        mock_statements_table.get_item.return_value = {
            "Item": {
                "statement_id": message["payload"].get("data", {}).get("object", {}).get("metadata", {}).get("statement_id", ""),
        ...
        with patch("core_app.workers.stripe_worker._table", side_effect=_table_selector), \
...
class TestLobArtifactAttachment:
    def test_rendered_pdf_url_attached(self):
        ...
        with patch("core_app.workers.lob_worker._table", side_effect=_table_selector), \
    ...
    def test_thumbnails_attached(self):
        ...
        lob_letter_id = _fake_lob_letter_id()
        event_id = f"evt_{uuid.uuid4().hex}"
        thumbnails = [{"small": "https://lob/thumb_s.png", "medium": "https://lob/thumb_m.png",
...
class TestOutboundPdfHash:
    def test_hash_mismatch_raises(self):
        ...
        with pytest.raises(ValueError, match="outbound_sha256 mismatch"):
            send_statement_letter(
                ...
                to_address={"name": "Jane Doe", "line1": "123 Main St", "city": "Portland",
                            "state": "OR", "zip": "97201"},
                from_address={"name": "Agency", "line1": "456 HQ Ave", "city": "Portland",
                              "state": "OR", "zip": "97201"},
            )
    ...
    def test_correct_hash_accepted(self):
        ...
        with patch("core_app.integrations.lob_service._get_lob_config") as mock_cfg, \
             patch("requests.post") as mock_post:
            mock_cfg.return_value = MagicMock(api_key="test_live_xxx")
            mock_resp = MagicMock()
            mock_resp.raise_for_status.return_value = None
            mock_resp.json.return_value = {"id": _fake_lob_letter_id(), "expected_delivery_date": "2026-03-05"}
