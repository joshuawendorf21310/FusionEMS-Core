...
async def test_claim_audit_logs_field_names_only() -> None:
    ...
    assert audits
    assert audits[-1].field_changes["changed_fields"] == ["payer_name", "version"]
...
@pytest.mark.asyncio
async def test_create_claim_idempotency_returns_existing_record_without_new_event() -> None:
    tenant_id = uuid.uuid4()
    actor_id = uuid.uuid4()
    existing = _build_claim(tenant_id=tenant_id, status=ClaimStatus.DRAFT, version=1)
