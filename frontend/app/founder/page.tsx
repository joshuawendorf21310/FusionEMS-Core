    ...
    <div className="bg-bg-panel border border-border-DEFAULT p-4" style={{ clipPath: 'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 0 100%)' }}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[rgba(255,255,255,0.82)]">{item.name}</span>
        </div>
        <span className="text-[10px] font-bold uppercase tracking-[0.14em]" style={{ color: statusColor }}>{item.status}</span>
      </div>
      <div className="mt-3 text-2xl font-bold text-text-primary">{item.configured_count}/{item.required_count}</div>
      <div className="mt-1 text-[11px] text-[rgba(255,255,255,0.42)]">Runtime checks configured</div>
      <div className="mt-3 min-h-[40px] text-[11px] text-[rgba(255,255,255,0.58)]">
        {item.missing_keys.length > 0 ? `Missing: ${item.missing_keys.join(', ')}` : 'Ready for runtime health checks.'}
      </div>
    </div>
  );
}
...
export default function FounderExecutivePage() {
  ...
  const [integrationReadiness, setIntegrationReadiness] = useState<{ integrations?: IntegrationReadinessItem[]; ready_count?: number; total_count?: number }>({});
  ...
  const quickLinks = [
    { label: 'Claims', href: '/billing/dashboard', detail: 'AR, denials, collections' },
    { label: 'Revenue', href: '/billing-command', detail: 'Stripe, subscriptions, reconciliation' },
    { label: 'Voice + SMS', href: '/founder/comms/phone-system', detail: 'Telnyx calls, alerts, messaging' },
    { label: 'Documents', href: '/billing/documents', detail: 'Statements, mail, attachments' },
    { label: 'Compliance', href: '/founder/compliance/nemsis', detail: 'NEMSIS, controls, filing readiness' },
    { label: 'Infrastructure', href: '/founder/infra/ecs', detail: 'Deployments, services, health' },
    { label: 'Systems', href: '/systems', detail: 'Registry, diagnostics, dependencies' },
    { label: 'AI Queue', href: '/founder/ai/review-queue', detail: 'Operator review and automation' },
  ];
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">
          {quickLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="bg-bg-panel border border-border-DEFAULT p-3 hover:border-[rgba(255,107,26,0.4)] transition-colors"
            >
              <div className="text-xs font-semibold text-text-primary">{link.label}</div>
              <div className="text-[10px] text-text-muted mt-0.5">{link.detail}</div>
            </Link>
          ))}
        </div>

        <div>
          <SectionHeader number="INT" title="Integration Readiness" sub={`${integrationReadiness.ready_count ?? 0}/${integrationReadiness.total_count ?? 3} vendors ready at runtime`} />
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {(integrationReadiness.integrations ?? []).map((item) => (
              <IntegrationReadinessCard key={item.name} item={item} />
            ))}
          </div>
        </div>
              ...
              <table className="w-full min-w-[600px] text-xs">
                <thead>
                  <tr>
                    ...
                    {(Object.keys(denialHeatmap[0]).filter((k: string) => k !== 'payer') as string[]).map((m: string) => (
                ...
                <tbody>
                  {denialHeatmap.map((row: any) => (
                    <tr key={row.payer}>
                      ...
                      {(Object.keys(row).filter((k: string) => k !== 'payer') as string[]).map((m: string) => (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="bg-[rgba(76,175,80,0.08)] border border-[rgba(76,175,80,0.2)] p-3 flex items-center gap-3" style={{ clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 0 100%)' }}>
              ...
              <div>
                <div className="text-xs font-semibold text-status-active">
                  {systemIncidents.length === 0
                    ? metrics?.mrr_cents != null
                      ? 'No active incidents'
                      : 'Incident status loading\u2026'
                    : `${systemIncidents.length} active incident${systemIncidents.length > 1 ? 's' : ''}`}
                </div>
                <div className="text-[11px] text-[rgba(255,255,255,0.4)]">Real-time from backend \u00b7 Last updated on load</div>
              </div>
            </div>
